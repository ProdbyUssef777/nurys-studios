...
export default function ReleaseRow({ release }: { release: Release }) {
  return (
    <div className="group grid grid-cols-[80px_1fr] items-center gap-5 border-b border-line py-6 md:grid-cols-[110px_1fr_auto] md:gap-8 md:py-8">
      <Link href={`/music/${release.slug}`} className="relative aspect-square w-full overflow-hidden bg-void">
        <Image
          src={release.cover}
      ...
      <div className="min-w-0">
        ...
        <Link href={`/music/${release.slug}`}>
          <h3 className="mt-1 truncate font-display text-2xl font-semibold tracking-tightest transition-colors hover:text-bone/70 md:text-3xl">
            {release.title}
          </h3>
        </Link>
